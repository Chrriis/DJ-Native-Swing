...
class Base64 {
  ...
  public static String encode(String s, boolean isURLSafe) {
      ...
      return encode(bytes, isURLSafe);
  }

  public static String encode(byte[] bytes, boolean isURLSafe) {
      int count = (bytes.length + 2) / 3 * 4 - (2 - ((bytes.length + 2) % 3));
      int padding = (bytes.length + 2) / 3 * 4 - count;
      char[] chars = new char[count + padding];
  ...
  public static String decode(String s) {
      try {
          int length = s.length();
          int count = (length + 2) / 4 * 3 - (2 - ((length + 2) % 4));
  ...
  public static void decode(String s, OutputStream out) throws IOException {
      ...
      for(int i=0; i<count; i++) {
          switch(i % 3) {
              case 0: {
                  int n = i / 3 * 4;
                  byte b = (byte)((fromChar(s.charAt(n)) << 2) | (fromChar(s.charAt(n + 1)) >> 4));
                  out.write(b);
                  break;
              }
              case 1: {
                  int n = i / 3 * 4 + 1;
                  byte b = (byte)(((fromChar(s.charAt(n)) & 0xF) << 4) | (fromChar(s.charAt(n + 1)) >> 2));
                  out.write(b);
                  break;
              }
              case 2: {
                  int n = i / 3 * 4 + 2;
                  byte b = (byte)(((fromChar(s.charAt(n)) & 0x3) << 6) | fromChar(s.charAt(n + 1)));
                  out.write(b);
                  break;
              }
          }
