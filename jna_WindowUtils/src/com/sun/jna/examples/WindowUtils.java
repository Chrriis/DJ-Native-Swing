...
public class WindowUtils {
  ...
  protected static class RepaintTrigger extends JComponent {
    ...
    protected class Listener extends WindowAdapter implements ComponentListener, HierarchyListener {
  ...
  public static abstract class NativeWindowUtils {
     ...
     * to remove the mask.
     */
    public void setWindowMask(Component w, Rectangle[] rectangles) {
      if (w.isLightweight())
    ...
    private class W32TransparentContent extends TransparentContent {
      ...
      protected void paintDirect(BufferedImage buf, Rectangle bounds) {
        ...
        try {
          ...
          blend.SourceConstantAlpha = level;
          blend.AlphaFormat = User32.AC_SRC_ALPHA;
          user.UpdateLayeredWindow(hWnd, screenDC, winLoc, winSize, memDC, srcLoc, 0, blend, User32.ULW_ALPHA);
        } finally {
      ...
      whenDisplayable(w, new Runnable() {
        public void run() {
          ...
          if (content instanceof W32TransparentContent) {
            ...
            W32TransparentContent w32content = new W32TransparentContent(content);
            ...
            if (rectangles == null) {
              ...
              try {
                for(int i=0; i<rectangles.length; i++) {
                  ...
                  gdi.SetRectRgn(tempRgn, rectangle.x, rectangle.y, rectangle.x + rectangle.width, rectangle.y + rectangle.height);
                  gdi.CombineRgn(result, result, tempRgn, GDI32.RGN_OR);
                }
              } finally {
                gdi.DeleteObject(tempRgn);
              }
  ...
  private static class X11WindowUtils extends NativeWindowUtils {
    private Pixmap createBitmap(final Display dpy, X11.Window win, Raster raster) {
    ...
    private X11.Window getContentWindow(Window w, X11.Display dpy, X11.Window win, Point offset) {
      ...
      whenDisplayable(w, new Runnable() {
        public void run() {
          ...
          if (content instanceof X11TransparentContent) {
            ...
            X11TransparentContent x11content = new X11TransparentContent(content);
    ...
    protected void setMask(final Component w, final Raster raster) {
      Runnable action = new Runnable() {
        public void run() {
          ...
          try {
            X11.Window win = getDrawable(w);
            if (raster == null || ((pm = createBitmap(dpy, win, raster)) == null)) {
              ext.XShapeCombineMask(dpy, win, X11.Xext.ShapeBounding, 0, 0, Pixmap.None, X11.Xext.ShapeSet);
            } else {
              ext.XShapeCombineMask(dpy, win, X11.Xext.ShapeBounding, 0, 0, pm, X11.Xext.ShapeSet);
            }
   ...
   * to remove the mask.
   */
  public static void setComponentMask(Component w, Rectangle[] rectangles) {
