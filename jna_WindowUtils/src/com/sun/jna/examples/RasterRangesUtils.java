...
/**
 * Methods that are useful to decompose a raster in ranges of contiguous
 * unoccupied pixels. An occupied pixel has two possible meanings, depending on
 * the raster :
 * <ul>
 * <li>if the raster has an alpha layer, occupied means with alpha not null</li>
 * <li>if the raster doesn't have any alpha layer, occupied means not
 * completely black</li>
 * </ul>
 * 
...
public class RasterRangesUtils {
  // / Masks used to isolate the current column in a set of 8 binary columns
  // packed in a byte
  private static final int[] subColMasks = new int[] { 0x0080, 0x0040, 0x0020, 0x0010, 0x0008, 0x0004, 0x0002, 0x0001 };
  ...
  /**
   * Outputs ranges of occupied pixels. In a raster that has an alpha layer, a
   * pixel is occupied if its alpha value is not null. In a raster without alpha
   * layer, a pixel is occupied if it is not completely black.
   * 
   * @param raster
   *          image to be segmented in non black or non-transparent ranges
   * @param out
   *          destination of the non null ranges
   * @return true if the output succeeded, false otherwise
   */
  public static boolean outputOccupiedRanges(Raster raster, RangesOutput out) {
    ...
    // Try to use the underlying data array directly for a few common raster
    // formats
    if (raster.getParent() == null && bounds.x == 0 && bounds.y == 0) {
      ...
      if (data.getNumBanks() == 1) {
        // There is always a single bank for all BufferedImage types, except
        // maybe TYPE_CUSTOM
        ...
        if (sampleModel instanceof MultiPixelPackedSampleModel) {
          ...
          // TODO could easily handle cases of TYPE_USHORT_GRAY and
          // TYPE_BYTE_GRAY.
        }
      }
    }
  ...
  /**
   * Output the non-null values of a binary image as ranges of contiguous
   * values.
   * 
   * @param binaryBits
   *          byte-packed binary bits of an image
   * @param w
  ...
  /**
   * Output the occupied values of an integer-pixels image as ranges of
   * contiguous values. A pixel is considered occupied if the bitwise AND of its
   * integer value with the provided occupationMask is not null.
   * 
   * @param pixels
   *          integer values of the pixels of an image
   * @param w
   *          width of the image (in pixels)
   * @param h
   *          height of the image
   * @param occupationMask
   *          mask used to select which bits are used in a pixel to check its
   *          occupied status. 0xff000000 would only take the alpha layer into
